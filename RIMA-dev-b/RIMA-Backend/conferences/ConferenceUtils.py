...
def get_conference_general_data(conference_name_abbr):
    ...
    print("*****"*50)
    print(result_data['series'][1]['data'])
    print("*****"*50)
...
'''
Those are the Functions implemented by Abdallah. some functions called inside of it are functions which are already implemented
'''  
####################################################################################################################################
# functions made by Abdallah
###################################################################################################################################
def get_top_words_in_years(list_of_events, keyword_or_topic):
    ...
    if keyword_or_topic == "keyword":
        for conference_event_name_abbr in list_of_events: 
            models_data = get_keywords_from_models(conference_event_name_abbr)
            models_data_total.append(models_data)
    if keyword_or_topic == "topic":
        for conference_event_name_abbr in list_of_events: 
            models_data = get_topics_from_models(conference_event_name_abbr)
            models_data_total.append(models_data)
    ...
    sorted_list_of_final_words ={k: v for k, v in sorted(list_of_final_words.items(), key=lambda item: item[1])}
...
def get_TotalSharedAuthors_between_conferences(conference_event_objs):
    ...
    for conference_event in conference_event_objs:
        check_exist = Author_has_Papers.objects.filter(conference_event_name_abbr =  conference_event.conference_event_name_abbr).exists()
        no_of_event_authors = Author_has_Papers.objects.filter(conference_event_name_abbr =  conference_event.conference_event_name_abbr).values_list('author_id').distinct().count()
        one_event_authors = Author_has_Papers.objects.filter(conference_event_name_abbr =  conference_event.conference_event_name_abbr).values_list('author_id').distinct()
        # one_event_authors_name = Author.objects.filter(conference_event_name_abbr =  conference_event.conference_event_name_abbr).values_list('author_name').distinct()
        if check_exist:
            result_data.append({
...
def get_shared_words_between_events(conference_events_list,keyword_or_topic):
    ...
    for conference_event in conference_events_list:
        ...
        print("conference_event_dataaaaaaaaaaaaaaaaaaa")
        print(conference_event_data)    
        for data in conference_event_data:
            # {'topic': 'Learners', 'weight': 9, 'event': 'aied2017'}
            all_words.append(data[keyword_or_topic])
    ...
    print(all_words)
    shared_words = list(set([word for word in all_words if all_words.count(word) == len(conference_events_list)]))
    print("here ARE THE SHARED WORRRDS")
    print(all_words)    
...
def get_shared_words_between_conferences(conferences_list,keyword_or_topic):
    ...
    for conference in conferences_list:
        ...
        print("here are the list of conferences")
        print(conference_event_objs)
        for conference_event in conference_event_objs:
            if keyword_or_topic == 'topic':
...
def add_data_to_conf_paper_and_author_models(conference_name_abbr,conf_event_name_abbr):
    ...
    for paper_data in data['paper_data']:
        key = 'paperId'
        if Conference_Event_Paper.objects.filter(paper_id = paper_data['paperId']).exists() or not (key in paper_data) :
            pass
        else:
